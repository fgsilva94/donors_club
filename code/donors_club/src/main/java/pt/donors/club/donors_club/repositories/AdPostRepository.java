...
public interface AdPostRepository extends CrudRepository<AdPost, Integer> {
    ...
    @Query(value = queryAdPostSimpleView + " WHERE ad.ad_id = :id", nativeQuery = true)
    Optional<AdPostSimpleView> findSimpleViewById(@Param("id") int id);
    ...
    @Query(value = queryAdPostSimpleView + " WHERE ad.ad_id = :adId AND ad.ad_owner_id = :usrId", nativeQuery = true)
    Optional<AdPostSimpleView> findSimpleViewByIdAndByOwner(@Param("adId") int adId, @Param("usrId") int usrId);
}
