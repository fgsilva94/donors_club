...
public interface ChatRepository extends CrudRepository<Chat, Integer> {
  String queryChatSimpleView = "SELECT c.chat_id as id, ad.ad_title as adTitle, IF(c.chat_usr_id = :id, ow.usr_name, u.usr_name) as user, "
                              ...
                              + "WHERE c.chat_usr_id = :id OR ad.ad_owner_id = :id";

  @Query(value = queryChatSimpleView, nativeQuery = true)
  Iterable<ChatSimpleView> findChatsByUser(@Param("id") int id);

  @Query(value = "SELECT chat_id AS id FROM chats WHERE chat_usr_id = :userId AND chat_ad_id = :adId", nativeQuery = true)
  Optional<ChatSimpleView> findChatByAdPostAndUser(@Param("userId") int userId, @Param("adId") int adId);
}
